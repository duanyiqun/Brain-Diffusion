...
from braindiffusion.utils.mne_visualize import visualize_eeg1020, visualize_eeg128, visualize_feature_map
...
class ZuCo_dataset(Dataset):
    def __init__(self, input_dataset_dicts, phase, tokenizer, subject = 'ALL', eeg_type = 'GD', bands = ['_t1','_t2','_a1','_a2','_b1','_b2','_g1','_g2'], setting = 'unique_sent', is_add_CLS_token = False, dataset_indices=2):
        self.inputs = []
        ...
        for idx, input_dataset_dict in enumerate(input_dataset_dicts):
            if subject == 'ALL':
                subjects = list(input_dataset_dict.keys())
                print('[INFO]using subjects: ', subjects)
            else:
                if idx == dataset_indices:
                    subjects = [subject]
                    print('[INFO]only using subjects: ', subjects)
                else:
                    subjects = list(input_dataset_dict.keys())
                    subjects = [subject]
                    print('[INFO]using subjects: ', subjects)
                ...
                if phase == 'train':
                    ...
                    for key in subjects:
                        for i in range(dev_divider,total_num_sentence):
                            ...
                            if input_sample is not None:
                                self.inputs.append(input_sample)
            elif setting == 'unique_subject_sent':
                # take first 80% as trainset, 10% as dev and 10% as test
                if phase == 'train':
                    print('[INFO]initializing a train set...')
                    for key in subjects:
...
if __name__ == '__main__':
    ...
    if check_dataset == 'ZuCo':
        ...
        dataset_path_task1 = '/projects/CIBCIGroup/00DataUploading/yiqun/bci/ZuCo/task1-SR/pickle/task1-SR-dataset.pickle' 
        with open(dataset_path_task1, 'rb') as handle:
            whole_dataset_dicts.append(pickle.load(handle))

        dataset_path_task2 = '/projects/CIBCIGroup/00DataUploading/yiqun/bci/ZuCo/task2-NR/pickle/task2-NR-dataset.pickle' 
        ...
        dataset_path_task2_v2 = '/projects/CIBCIGroup/00DataUploading/yiqun/bci/ZuCo/task2-NR-2.0/pickle/task2-NR-2.0-dataset.pickle' 
        ...
        print('devset size:',len(dev_set))
        print('testset size:',len(test_set))
        for input_embeddings, seq_len, input_attn_mask, input_attn_mask_invert, target_ids, target_mask, sentiment_label, sent_level_EEG in dev_set:
            print(input_embeddings.size())
            pil_image  = visualize_feature_map(input_embeddings.numpy())
