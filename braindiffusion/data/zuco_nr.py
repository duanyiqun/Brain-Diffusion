...
# macro
ZUCO_SENTIMENT_LABELS = json.load(open('/projects/CIBCIGroup/00DataUploading/yiqun/bci/ZuCo/task1-SR/sentiment_labels/sentiment_labels.json'))
SST_SENTIMENT_LABELS = json.load(open('/projects/CIBCIGroup/00DataUploading/yiqun/bci/ZuCo/stanfordsentiment/ternary_dataset.json'))
...
def pad_to_max_sample(input_tensor, max_len = 8350):
    input_tensor = input_tensor.float()
    channel, sample_len = input_tensor.shape
    if channel is not 105:
        input_tensor = input_tensor.permute(1,0)
    channel, sample_len = input_tensor.shape
    # print(input_tensor.shape)
    if sample_len < max_len:
        pad_len = max_len - sample_len
        # print(pad_len)
        input_tensor = np.pad(input_tensor, pad_width=((0,0), (0, pad_len)), mode='constant', constant_values=(0,0))
    else:
        input_tensor = input_tensor[:,:max_len].numpy()
...
def get_input_sample(sent_obj, tokenizer, eeg_type = 'GD', bands = ['_t1','_t2','_a1','_a2','_b1','_b2','_g1','_g2'], max_len = 56, add_CLS_token = False):
    ...
    def get_word_embedding_eeg_tensor(word_obj, eeg_type, bands):
        rawEEG = word_obj['word_level_EEG']['rawEEG']
        
        # word_eeg_embedding = np.concatenate(rawEEG, axis=0)
        # assert len(word_eeg_embedding) == 105*len(bands)
        if type(rawEEG) is float:
            return None
        return_tensor = torch.from_numpy(rawEEG)
        return normalize_1d(return_tensor)

    def get_sent_eeg(sent_obj, bands):
        sent_eeg_features = []
        rawEEG = sent_obj['sentence_level_EEG']['rawData']
        # print(rawEEG.shape)
        if type(rawEEG) is float:
            return None
        # print(rawEEG.shape)
    ...
    if sent_level_eeg_tensor is None:
    ...
    input_sample['input_embeddings'] = None # max_len * (105*num_bands)
...
if __name__ == '__main__':
    ...
    if check_dataset == 'ZuCo':
        ...
        dataset_path_task1 = '/projects/CIBCIGroup/00DataUploading/yiqun/bci/ZuCo/dewave_sent/ZuCo/task1-SR/pickle/task1-SR-dataset.pickle' 
        with open(dataset_path_task1, 'rb') as handle:
            whole_dataset_dicts.append(pickle.load(handle))
        
        dataset_path_task2 = '/projects/CIBCIGroup/00DataUploading/yiqun/bci/ZuCo/dewave_sent/ZuCo/task2-NR/pickle/task2-NR-dataset.pickle' 
        with open(dataset_path_task2, 'rb') as handle:
            whole_dataset_dicts.append(pickle.load(handle))
        
        dataset_path_task3 = '/projects/CIBCIGroup/00DataUploading/yiqun/bci/ZuCo/dewave_sent/ZuCo/task3-TSR/pickle/task3-TSR-dataset.pickle' 
        with open(dataset_path_task3, 'rb') as handle:
            whole_dataset_dicts.append(pickle.load(handle))
        
        dataset_path_task2_v2 = '/projects/CIBCIGroup/00DataUploading/yiqun/bci/ZuCo/dewave_sent/ZuCo/task2-NR-2.0/pickle/task2-NR-2.0-dataset.pickle' 
        ...
        tokenizer = BertTokenizerFast.from_pretrained('facebook/bert-base-uncased')
        ...
        for seq_len, input_masks, input_mask_invert, target_ids, target_mask, sentiment_labels, sent_level_EEG in dev_set:
