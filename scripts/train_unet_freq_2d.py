...
from braindiffusion.utils.mne_visualize import stack_topomaps
    ...
    for epoch in range(args.num_epochs):
        ...
        if accelerator.is_main_process:
            ...
            if (epoch + 1) % args.save_images_epochs == 0:
                ...
                images, (sample_rate, topomaps) = pipeline(
                    generator=generator,
                ...
                for index, image in enumerate(images):
                    ...
                    image.save(os.path.join(os.path.join(visualization_dir, f"epoch{epoch}"), f"featuremap_images:{epoch}_{band_names[index]}.png"))    
                
                for index, topo in enumerate(topomaps):
                    file_name = os.path.join(visualization_dir, f"epoch{epoch}", f"topomap_images:{epoch}_{index}.png")
                    stack_topomaps(topo, file_name)
                    file_name = os.path.join(visualization_dir, f"epoch{epoch}", f"topomap_images:{epoch}_{index}.gif")
                    gif.save(topo, file_name, duration=1000)
